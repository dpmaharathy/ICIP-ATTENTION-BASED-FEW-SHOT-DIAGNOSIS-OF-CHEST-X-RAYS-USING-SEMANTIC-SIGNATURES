...
def train(eps, model, train_x, train_y, val_x, val_y, n_way, n_support, n_query, max_epoch, epoch_size, temp_str):
# def train(model, train_x, train_y, val_x, val_y, n_way, n_support, n_query, max_epoch, epoch_size, temp_str):
  ...
  Emb_optim = torch.optim.Adam(Emb.parameters(), lr = 0.001)
  RN_optim = torch.optim.Adam(RN.parameters(), lr = 0.001)
...
for i in range(1):
    ...
    max_epoch = 20
    epoch_size = 100
    temp_str = path+'nih_relationnet_' + str(i)
    train(0.005, model, X, y, X_val,y_val, n_way, n_support, n_query, max_epoch, epoch_size,temp_str)
...
for j in range(1):
    ...
    test_episode = 1000
    lock = 1
    temp_str = path+'nih_relationnet_' + str(j)
...
mean_pre = np.round(np.mean(pre, axis=0), decimals=2)
mean_re = np.round(np.mean(re, axis=0), decimals=2)
mean_f1 = np.round(np.mean(f1, axis=0), decimals=2)

print(mean_pre, "\n", mean_re,"\n", mean_f1)

std_pre = np.round(np.std(pre, axis=0), decimals=2)
std_re = np.round(np.std(re, axis=0), decimals=2)
std_f1 = np.round(np.std(f1, axis=0), decimals=2)

print(std_pre, "\n", std_re,"\n", std_f1)

mean_auroc = []
mean_auroc.append(np.round(np.mean(arr_1),2))
mean_auroc.append(np.round(np.mean(arr_2),2))
mean_auroc.append(np.round(np.mean(arr_3),2))

std_auroc = []
std_auroc.append(np.round(np.std(arr_1),2))
std_auroc.append(np.round(np.std(arr_2),2))
std_auroc.append(np.round(np.std(arr_3),2))
